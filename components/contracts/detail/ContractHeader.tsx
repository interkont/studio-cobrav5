...
const ContractHeader: React.FC<Props> = ({ data }) => {
      ...
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <div className="flex items-center gap-3 mb-3">
            <h1 className="text-3xl font-extrabold tracking-tight text-cobra-text-main">
              {data.title} <span className="text-cobra-text-secondary font-normal ml-1">#{data.id}</span>
            </h1>
          </div>
          <div className="flex gap-2">
            <span className="px-3 py-1 bg-green-50 text-green-700 rounded-full text-[12px] font-black border border-green-100 flex items-center gap-1.5 shadow-sm">
        ...
        <div className="bg-cobra-slate text-white p-5 px-7 rounded-3xl shadow-2xl flex flex-col items-end min-w-[280px] border border-white/5 relative overflow-hidden group">
          ...
          <div className="text-[10px] uppercase font-black text-slate-400 mb-1.5 tracking-widest flex items-center gap-2">
             TIEMPO PARA FINALIZACIÓN <i className="ph ph-hourglass-high animate-spin-slow"></i>
          </div>
